...
const HomeScreen = () => {
    ...
    const [dbStatus, setDbStatus] = useState<'loading' | 'ok' | 'failed'>('loading');

    useEffect(() => {
        // Check server status
        fetch(SERVER_URL)
            .then((res) => res.ok ? setServerStatus('ok') : setServerStatus('failed'))
    ...
    return (
        <View style={styles.container}>
            ...
            {serverStatus === 'loading' ? <ActivityIndicator /> :
                <Text style={[styles.status, serverStatus === 'ok' ? styles.ok : styles.failed]}>
                    {serverStatus === 'ok' ? 'OK' : 'FAILED'}
                </Text>
            }

            <Text style={styles.subtitle}>Checking DB status...</Text>
            {dbStatus === 'loading' ? <ActivityIndicator /> :
                <Text style={[styles.status, dbStatus === 'ok' ? styles.ok : styles.failed]}>
